...
class LogisticRegression:
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        if append_bias:
            X = LogisticRegression.append_biases(X)
        y_predictions = self.activate_func(X, self.w)
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        y_predictions = self.activate_func(X_batch, self.w)
    ...
    def activate_func(self, x, w):
        return self.sigmoid(x.dot(w.T))

    def gradient(self, w, x, t):
        return (self.activate_func(x, w) - t).T * x
